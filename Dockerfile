...
RUN apk upgrade --no-cache && apk add --no-cache "clamav=${CLAMAV_PACKAGE_VERSION}" "clamav-daemon=${CLAMAV_PACKAGE_VERSION}" coreutils ffmpeg findutils python3 util-linux tzdata && clamd --version | grep -Eq '^ClamAV 1\.4\.5($|/)' && ffprobe -version >/dev/null && addgroup -S -g 10001 clamav-scheduled && adduser -S -D -u 10001 -G clamav-scheduled -h /home/clamav-scheduled clamav-scheduled && install -d -o 10001 -g 10001 -m 0750 /config /downloads /events /home/clamav-scheduled /quarantine /state /tmp/clamav-runtime /var/log/clamav && chmod 0755 /var/lib/clamav
