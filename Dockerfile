...
RUN chmod 0555 /usr/local/bin/clamav_scheduled.sh /usr/local/bin/clamd_session_scan.py /usr/local/bin/scan_list_filter.py /usr/local/bin/clamav_healthcheck.py /usr/local/bin/clamav_entrypoint.py /usr/local/bin/clamav_ui_server.py && chmod -R a=rX /usr/local/share/clamav-ui

ENV APP_MODE=headless HOME=/home/clamav-scheduled PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1 RUNTIME_DIR=/tmp/clamav-runtime DEFINITIONS_DIR=/var/lib/clamav DEFINITIONS_MAX_AGE_SECONDS=172800
